...
export const verifyCleanupTask = async (req, res, next) => {
  try {
    ...
    const proximityQuery = `
      ...
      LEFT JOIN cleanup_tasks t ON t.report_id = r.id
      WHERE r.id = $3
      LIMIT 1
    `;

    const result = await query(proximityQuery, [longitude, latitude, report_id]);
    ...
    const distanceMeters = parseFloat(taskData.distance_meters || 0);
    const isWithinProximity = distanceMeters <= MAX_CLEANUP_PROXIMITY_METERS;
    const penaltyPoints = 25;
    const fraudReason = `Fraudulent cleanup submission detected: GPS distance (${distanceMeters.toFixed(1)}m) exceeded the 10m tolerance threshold.`;

    await query('BEGIN');

    if (isWithinProximity) {
      await query("UPDATE civic_reports SET status = 'VERIFIED' WHERE id = $1", [report_id]);
      ...
      await query(
        `UPDATE cleanup_tasks
         ...
         WHERE report_id = $5`,
        [after_image_url, longitude, latitude, distanceMeters, report_id]
      );
      ...
      return res.status(200).json({
        ...
        proximity_metrics: {
          distance_meters: Number((Math.round(distanceMeters * 100) / 100).toFixed(2)),
          tolerance_threshold_meters: MAX_CLEANUP_PROXIMITY_METERS,
        },
        reward: {
          trust_score_delta: '+10 Points',
        },
        message: 'Cleanup verified! GPS location matched the original site within 10 meters.',
      });
    }
    ...
    await query(
      `UPDATE cleanup_tasks
       ...
       WHERE report_id = $6`,
      [after_image_url, longitude, latitude, distanceMeters, fraudReason, report_id]
    );
    ...
    await query(
      `INSERT INTO slashing_logs (user_id, task_id, penalty_points, reason, logged_at)
       VALUES ($1, $2, $3, $4, NOW())`,
      [ngoId, taskData.task_id || null, penaltyPoints, fraudReason]
    );
